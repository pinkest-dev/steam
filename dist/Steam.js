...
class Steam extends Base {
    ...
    async getConfirmations(identity_secret) {
        try {
    ...
    async acceptManyConfirmations(cid, cKey, identity_secret) {
        try {
    ...
    async acceptConfirmation(cid, cKey, identity_secret) {
        try {
            if (!this.getMySteamid64() || !identity_secret) {
                throw new Error("Must have steamid+identity_secret before to do anything with confirmations");
            }
    ...
    getConfirmationKey(identity_secret, time, tag) {
        try {
            let data_len = 8;
            if (tag)
                if (tag.length > 32)
    ...
    */
    ...
    async getReceivedOffers(steamApikey) {
        try {
